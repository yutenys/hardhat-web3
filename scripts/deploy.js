...
async function main() {
  const Lock = await hre.ethers.getContractFactory("Lock_eth");
  const lock = await Lock.deploy();
  await lock.deployed();
  console.log(
    `deployed lock_eth to ${lock.address}`
  );
  const Lock2 = await hre.ethers.getContractFactory("Lock_eth2");
  const lock2 = await Lock2.deploy();
  await lock2.deployed();
  console.log(
    `deployed lock_eth2 to ${lock2.address}`
  );
